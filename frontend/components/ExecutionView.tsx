 ...
 */
export function ExecutionView({ code, result, index, error, isLoading }: ExecutionViewProps) {
  return (
        ...
        {result?.ok && result.steps.length > 0 && (
          <div className="mb-4">
            <ExplainPanel
              ...
              prevStep={index > 0 ? result.steps[index - 1] : null}
            />
          </div>
        )}
        <Body code={code} result={result} index={index} error={error} isLoading={isLoading} />
      </div>
